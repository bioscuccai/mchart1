...
*/
  ...
  private resetInitialExtent(){
    //this.setInitialExtent();
    let oldExtent=this.timeBrush.extent();
    /*let newExtent=[
      _.max([_.min(this.data.map(e=>e.timestamp)), oldExtent[0]]),
      _.min([_.max(this.data.map(e=>e.timestamp)), oldExtent[1]])
    ];*/
    let newExtent=[
      _.min(this.data.map(e=>e.timestamp)),
      _.max(this.data.map(e=>e.timestamp))
    ];
    this.timeBrush.extent(newExtent);
    //this.timeBrush(this.timeBrush.extent());
    //this.timeBrush.extent(this.initialExtent);
    this.timeBrush(d3.select('#brush'));
    this.timeBrush.event(this.brushSvg);
  }
